...
class EagleeduExam(models.Model):
    ...
    subject_line = fields.One2many('eagleedu.subject.line', 'exam_id', string='Subjects')
    ...
    def get_class_domain(self):
        for rec in self:
            domain=[]
            existing_class=self.env['eagleedu.exam'].search([('exam_type.id','=',rec.exam_type.id),('academic_year.id','=',rec.academic_year.id)])
            for cls in existing_class:
                domain.append(cls.class_id.id)
        return {'domain': {'class_id': [('id', '!=', domain)]}}
    @api.model
    def create(self, vals):
        res = super(EagleeduExam, self).create(vals)
        if res.division_id:
    ...
    def close_exam(self):
        all_completed=1
        valuation_status=self.env['eagleedu.exam.valuation'].search([('exam_id','=',self.id)])
    ...
    def check_student_section_subject(self,section_id,subject_id):
        sections=self.env['eagleedu.class.division'].search([('academic_year_id','=',self.academic_year.id),('class_id','=',self.class_id.id)])
        for section in sections:
            if section.id==section_id:
                section_in_history=self.env['eagleedu.class.history'].search([('class_id.id', '=', section.id)])
        ...
        for section in sections:
            for subject in subjects:
                ...
                if check_student==True:
                    ...
                    self.env['eagleedu.exam.valuation'].create({
                        'exam_id':self.id,
    ...
    def get_subjects(self):
        for rec in self:
            subjline_obj=self.env['eagleedu.subject.line']

            subjects=self.env['eagleedu.syllabus'].search([('class_id','=',rec.class_id.id),('academic_year','=',rec.academic_year.id)])  #.search([('class_id', '=', self.id)])
            for subject in subjects :
    ...
    def print_results(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'eagleedu.exam.result.wizard',
            'target': 'new',
...
class SubjectLine(models.Model):
    _name = 'eagleedu.subject.line'
    _rec_name = 'subject_id'
    subject_id = fields.Many2one('eagleedu.syllabus', string='Subject', required=True)
    ...
    exam_id = fields.Many2one('eagleedu.exam', string='Exam')
    company_id = fields.Many2one('res.company', string='Company',
                                 default=lambda self: self.env['res.company']._company_default_get())
...
class examlist(models.AbstractModel):
    ...
    batch=fields.Many2one('eagleedu.academic.year',"batch")
    class_id= fields.Many2one('eagleedu.class',"Class")
    group= fields.Many2one('eagleedu.division',"Group")
    section= fields.Many2one('eagleedu.class.section',"Section")
    subject= fields.Many2one('eagleedu.syllabus',"Subject")
    exam_type= fields.Many2one('eagleedu.exam.type',"Exam")
