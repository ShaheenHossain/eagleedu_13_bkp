...
class EagleeduExamResults(models.Model):
    _name = 'eagleedu.exam.results'
    ...
    def calculate_result(self,exam_id):
        results = self.env['eagleedu.exam.results'].search([('exam_id','=',exam_id.id)])
        for result in results:
            if result.state!='done':
                ...
                for subject in result.subject_line:
                    student_history = self.env['eagleedu.class.history'].search(
...
class ResultsSubjectLine(models.Model):
    ...
    name = fields.Char(string='Name')
    subject_id = fields.Many2one('eagleedu.syllabus', string='Subject')
    ...
    result_id = fields.Many2one('eagleedu.exam.results', string='Result Id')
    exam_id = fields.Many2one('eagleedu.exam', string='Exam')
    class_id = fields.Many2one('eagleedu.class', string='Class')
    division_id = fields.Many2one('eagleedu.class.division', string='Division')
    student_id = fields.Many2one('eagleedu.student', string='Student')
...
class exam_result_summery(models.Model):
    _name = 'eagleedu.exam.student.summary'
    exam_id =fields.Many2one('eagleedu.exam','Exam')
    student_id=fields.Many2one('eagleedu.student',string='Student')
    total_mark=fields.Integer('Total')
    total_gpa=fields.Many2one('eagleedu.exam.grading', 'Total GPA')
    total_lg=fields.Integer( 'Total GPA')
    additional_mark=fields.Integer('additional')
    additional_gpa = fields.Many2one('eagleedu.exam.grading', 'additional GPA')
    additional_lg = fields.Integer( 'additional Grade')
    extra_mark=fields.Integer('Extra')
    extra_gpa = fields.Many2one('eagleedu.exam.grading', 'Extra GPA')
    extra_lg = fields.Integer('Extra Grade')
    net_mark=fields.Integer('net')
    net_gpa = fields.Many2one('eagleedu.exam.grading', 'NET GPA')
