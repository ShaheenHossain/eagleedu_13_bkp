...
class EagleeduExamValuation(models.Model):
    ...
    exam_id = fields.Many2one('eagleedu.exam', string='Exam', required=True, domain=[('state', '=', 'ongoing')])
    class_id = fields.Many2one('eagleedu.class', string='Class', required=True)
    division_id = fields.Many2one('eagleedu.class.division', string='Division', required=True)
    ...
    def domain4subject(self):
        ...
        for rec in self:
            if rec.division_id.id:
                result_created=self.env['eagleedu.exam.valuation'].search([('exam_id.id','=',rec.exam_id.id),('division_id.id','=',rec.division_id.id)])
...
class StudentsExamValuationLine(models.Model):
    ...
    valuation_id = fields.Many2one('eagleedu.exam.valuation', string='Valuation Id')
